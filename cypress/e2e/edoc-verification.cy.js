...
describe("eDoc Parser and Verification Tests", () => {
  it("should parse and verify a real eDoc file", () => {
    cy.task("fileExists", "sensitive/Sample File.edoc").then((exists) => {
      ...
      cy.task("readBinaryFile", "sensitive/Sample File.edoc").then(
        async (base64Content) => {
          ...
          cy.log(
            `- Canonicalization Method: ${signature.canonicalizationMethod || "default"}`,
          );
          ...
          cy.log(
            `- Certificate valid: ${verificationResult.certificate.isValid}`,
          );

          if (verificationResult.signature) {
            cy.log(
              `- XML Signature valid: ${verificationResult.signature.isValid}`,
            );
            if (
