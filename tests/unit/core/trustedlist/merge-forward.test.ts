...
const previous: CompactTrustedListBundle = {
  ...
  sources: [["eu", "EU LOTL", "https://example.test/eu-lotl.xml"]],
  dns: ["CN=Old LV,C=LV", "CN=Old EE,C=EE"],
  services: [
...
describe("mergeForwardUnreachableTerritories", () => {
  it("carries forward services for territories missing from the fresh bundle", () => {
    const merged = mergeForwardUnreachableTerritories(fresh, previous);
  ...
  it("returns the fresh bundle unchanged when nothing is missing", () => {
    const merged = mergeForwardUnreachableTerritories(fresh, fresh);
