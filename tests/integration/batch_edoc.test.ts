...
const PUBLIC_SAMPLE_CHECKLIST_EXPECTATIONS: Record<string, PublicSampleChecklistExpectation> = {
  ...
  "terms_of_Account_Agreement_28062025_ENG.asice": {
    ...
    trustListMatch: {
      ...
      source: "eu",
      sourceLabel: "EU LOTL",
      detailIncludes: "exact issuer SPKI match",
    },
  },
...
function getExpectedTrustChecklistStatus(result: VerificationResult) {
  ...
  if (result.trustListMatch.trustedAtSigningTime === true) {
    return "pass";
  }

  if (result.trustListMatch.trustedAtSigningTime === false || !result.trustListMatch.found) {
    return "fail";
  }
        ...
        results.forEach(({ signerName, result }, index) => {
          ...
          if (expectedChecklist) {
            ...
            expect(result.trustListMatch).toMatchObject({
              ...
              source: expectedChecklist.trustListMatch.source,
              sourceLabel: expectedChecklist.trustListMatch.sourceLabel,
            });
